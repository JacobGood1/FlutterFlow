...
class HelloWorldSupabaseUser extends BaseAuthUser {
  HelloWorldSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> helloWorldSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HelloWorldSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
